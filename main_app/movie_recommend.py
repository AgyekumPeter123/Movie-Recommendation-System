...
if dark_mode:
    ...
    # White for input label in Dark Mode
    ...
    # Deep Red for input label in Light Mode
...
st.markdown(f"""
    ...
    /* 3. FORCE SIDEBAR TEXT BLACK (BUT NOT MAIN PAGE) */
    ...
    /* 5. GENERAL MAIN LABELS */
    .main label {{
        color: {label_color} !important;
    }}

    /* ============================================================ */
    /* 🚀 DYNAMIC COLOR FOR SELECTBOX LABEL + GLOW ANIMATION 🚀 */
    /* ============================================================ */
    
    @keyframes glow {{
        from {{ text-shadow: 0 0 2px {input_label_color}; }}
        to {{ text-shadow: 0 0 10px #FF4B4B; }}
    }}

    /* Target direct label child of the selectbox div */
    div[data-testid="stSelectbox"] > label {{
        color: {input_label_color} !important;  /* DYNAMIC COLOR */
    ...
    /* ============================================================ */
    /* 💓 HEADER PULSE ANIMATION ON HOVER 💓 */
    /* ============================================================ */

    @keyframes pulse-header {{
        0% {{ transform: scale(1); }}
        50% {{ transform: scale(1.05); text-shadow: 0 0 15px rgba(255, 75, 75, 0.6); }}
    ...
    /* 6. RECOMMENDATION CARDS (RESTORED HOVER) */
    .rec-card {{
        background: {card_bg};
        ...
        transition: transform 0.3s ease, box-shadow 0.3s ease; /* RESTORED ANIMATION */
    }}
    
    /* RESTORED HOVER STATE */
    .rec-card:hover {{
        transform: translateY(-5px);
        box-shadow: 0 12px 20px rgba(255, 75, 75, 0.2);
        border-color: #FF4B4B;
    }}

    /* 7. HEADER LOGO */
    header[data-testid="stHeader"]::after {{
        content: 'GROUP 4 2025 SEPTEMBER COHORT';
    ...
    [data-testid="stImage"] img {{
        ...
        object-fit: cover;
        border-radius: 20px;
        box-shadow: 0 12px 24px rgba(255, 75, 75, 0.4);
        margin-bottom: 20px;
    }}
    </style>
""", unsafe_allow_html=True)
...
def get_recommendations(movie, method):
    try:
        ...
        for i in scores:
            if method == 'Content-Based Filtering':
                result.append(titles.iloc[i[0]].title)
            else:
                result.append(collab_titles[i[0]])
        return result
    except:
        return ["Movie not found or error in database"]
...
st.markdown('<h1 class="main-header" style="text-align: center; color: #FF4B4B; font-size: 3.5rem; font-weight: 800;">🍿 Movie Recommender</h1>', unsafe_allow_html=True)
